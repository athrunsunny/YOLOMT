...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=(), eps=1e-16, prefix="", suffix=""):
    ...
    if plot:
        plot_pr_curve(px, py, ap, Path(save_dir) / f'{prefix}PR_curve_{suffix}.png', names)
        plot_mc_curve(px, f1, Path(save_dir) / f'{prefix}F1_curve_{suffix}.png', names, ylabel='F1')
        plot_mc_curve(px, p, Path(save_dir) / f'{prefix}P_curve_{suffix}.png', names, ylabel='Precision')
        plot_mc_curve(px, r, Path(save_dir) / f'{prefix}R_curve_{suffix}.png', names, ylabel='Recall')
...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir='', names=(), suffix=''):
        try:
            ...
            fig.savefig(Path(save_dir) / f'confusion_matrix_{suffix}.png', dpi=250)
            plt.close()
...
def select_iouloss_type(iou_type):
    """
    select iou loss type,see function bbox_iou()
    """
    parms = inspect.signature(bbox_iou).parameters
    kwargs = {name: parm.default for name, parm in parms.items() if not isinstance(inspect._empty, type(parm.default))}

    for name, parm in kwargs.items():
        if name.lower() in iou_type or name in iou_type or name.upper() in iou_type:
            for item, value in iou_type.items():
                if name.lower() == item or name == item or name.upper() == item:
                    kwargs[name] = value
    return kwargs
