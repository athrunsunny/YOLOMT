...
def reshape_classifier_output(model, n=1000):
    ...
    from models.multask_common import Classify
...
class CEarlyStopping:
    # YOLOv5 simple early stopper
    def __init__(self, patience=30, close_mosaic=0, epochs=300):
        self.best_fitness = 0.0  # i.e. mAP
        self.best_epoch = 0
        self.patience = patience or float('inf')  # epochs to wait after fitness stops improving to stop
        self.possible_stop = False  # possible stop may occur next epoch
    ...
    def __call__(self, epoch, fitness):
        ...
        if self.close_mosaic == 0:
            ...
            if not self.change_data:
                buffer = self.patience - 1 - self.close_mosaic
                self.change_data = delta >= buffer or self.epochs - epoch <= self.close_mosaic  # possible change not mosaic datasets next epoch
                self.start_change = epoch
            stop = epoch - self.start_change >= self.close_mosaic
            if stop:
                LOGGER.info(f'Stopping training early with close mosaic at last {self.close_mosaic} epochs. ')
            return stop
